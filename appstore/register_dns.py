#!/usr/bin/env python3
"""Register the App Store as a service in re-ddns.
...
Environment variables (all optional — sensible defaults provided):
    ...
    SERVICE_SUBDOMAIN    – subdomain to register             (aapps)
    SERVICE_ZONE         – DNS zone                          (reflex-ddns.com)
    SERVICE_IP           – IP the A record should point to   (auto-detected)
"""
...
SUBDOMAIN = os.environ.get("SERVICE_SUBDOMAIN", "aapps")
...
def main() -> None:
    ...
    ok = register_service()
    if ok:
        print(f"[register] OK: {SUBDOMAIN}.{ZONE}")
    else:
        print("[register] WARNING: registration failed – continuing anyway.")
