...
_REFLEX_BACKEND_PATHS = (
    "/_event", "/ping", "/_upload", "/_health", "/api",
)
...
def _server_blocks_resolver(
    ...
    all_backend_paths = _REFLEX_BACKEND_PATHS + extra_backend_paths
...
def _service_config(svc: dict) -> str:
    ...
    # Per-service extra backend paths (declared in project's backend-paths.txt)
    extra = tuple(svc.get("backend_paths") or [])

    lines: list[str] = [
        f"# Auto-generated by re_ddns for {fqdn}",
        "",
        _server_blocks_resolver(fqdn, sub, fe_target, be_target, extra),
    ]
    return "\n".join(lines)
